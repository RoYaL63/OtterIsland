...
final class MonitorWindowController {
    ...
    init(monitor: SystemMonitor, memory: MemoryMonitor, battery: BatteryMonitor) {
    ...
    private func makeWindow() -> NSWindow {
        let root = MonitorWindowView(monitor: monitor, memory: memory, battery: battery)
...
struct MonitorWindowView: View {
    ...
    private var header: some View {
        HStack(spacing: 18) {
            ...
            metric("Mémoire", "\(Int(memory.usedFraction * 100)) %", tint: memory.usedFraction > 0.9 ? .orange : .primary)
            metric("Charge", String(format: "%.2f", monitor.loadRatio).replacingOccurrences(of: ".", with: ","), tint: monitor.loadRatio > 1 ? .orange : .primary)
            Spacer()
        }
    ...
    private var footer: some View {
        HStack(spacing: 10) {
            ...
            Button("Actualiser") { monitor.refresh() }
            Spacer()
